...
- [x] **Admin UI:** Ghi `ban_reason` vào `child_profiles` khi khoá (kèm lý do trong `admin_audit_log`). Hiện lý do khi rê chuột vào nhãn *Đã khoá*; mở khoá thì xoá lý do — lịch sử đầy đủ vẫn ở audit log.
...
Kiểm chứng: `D-2` báo *"chưa khoá nào bị chỉnh khác mặc định"* → code và DB đã khớp nhau.
