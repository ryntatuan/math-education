...
import { setGuestMode, clearPrivateKeys } from "./sessionMode";
import soundManager from "../utils/soundManager";
const GOOGLE_WEB_CLIENT_ID =
  "974832122244-8pkn02h3puctsl4fcpc60ig28k421a4u.apps.googleusercontent.com";
...
export const useAuthStore = create((set, get) => ({
  ...
  initAuth: async () => {
    ...
    try {
      ...
      if (session?.user) {
        ...
        const result = await syncService.autoMigrateGuestDataToCloud(
          session.user,
        );
        ...
        // 🔴 Chỉ "khách không ghi gì" là CHƯA ĐỦ: máy có thể còn dữ liệu ghi từ bản trước
        // (hoặc của người dùng trước trên máy dùng chung). Không dọn thì khách sẽ nạp lại
        // đúng dữ liệu đó và nhìn thấy Xu / tiến độ của người khác.
        // An toàn: nhánh này chỉ chạy khi Supabase ĐÃ cấu hình — nhánh "chưa cấu hình"
        // đã `return` ở đầu `initAuth`, nên không bao giờ xoá chỗ lưu duy nhất của app.
        clearPrivateKeys();
      }
  ...
  signInWithGoogle: async () => {
    if (!isSupabaseConfigured() || !supabase) {
      alert(
        "Chưa cấu hình Supabase! Vui lòng cấu hình VITE_SUPABASE_URL và VITE_SUPABASE_ANON_KEY trong file .env để dùng tính năng này.",
      );
      return;
    }
    ...
    try {
      ...
      if (Capacitor.isNativePlatform()) {
        ...
        try {
          ...
          console.warn(
            "Lỗi đăng nhập Native, chuyển sang dự phòng Custom Tab:",
            nativeErr,
          );
...
// 🔴 Một chỗ DUY NHẤT quyết định chế độ Khách, thay vì gọi `setGuestMode` rải khắp 6 nhánh
// đăng nhập/đăng xuất trong file này — mỗi nhánh quên một lần là dữ liệu riêng còn nằm
// lại trên máy. `isGuest` khởi tạo là `true` nên lúc mới mở app CHƯA ghi gì; xác nhận
// có phiên thật thì mới cho ghi (xem `sessionMode.js`).
setGuestMode(useAuthStore.getState().isGuest);
useAuthStore.subscribe((state) => setGuestMode(state.isGuest));
