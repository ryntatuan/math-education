...
export default function HomePage() {
  ...
  const currentGradeData =
    curriculum.grades.find((g) => g.id === grade) || curriculum.grades[0];
  ...
  const displayedChaptersWithIndex = allChaptersForGrade
    ...
    <div className="home-dashboard-container">
      <div className="page-2col-layout">
        ...
        <section className="learning-main-column">
          ...
          <div className="home-semester-bar">
            <div className="home-semester-tabs">
              ...
              >
                ...
                <span className="hide-desktop hide-tablet">
                  ⭐ Tất cả ({allChaptersForGrade.length})
                </span>
              </button>
              <button
                type="button"
              ...
              >
                <span className="hide-mobile">
                  ☀️ Học kỳ 2 (Chương {halfPoint + 1} -{" "}
                  {allChaptersForGrade.length})
                </span>
                <span className="hide-desktop hide-tablet">
                  ☀️ HK 2 ({halfPoint + 1} - {allChaptersForGrade.length})
                </span>
              </button>
            </div>
          </div>
          ...
          >
            {displayedChaptersWithIndex.map(
              ({ chapter, originalIndex }, mapIndex) => {
                const lessonIds = (chapter.lessons ?? []).map((l) => l.id);
                // 🔴 `??` chứ KHÔNG phải `||`. Chương đã bị xoá hết bài có
                // `lessons.length === 0`, mà `0 || 12` ra **12** ⇒ chương trống lại
                // hiện "0/12 bài". Lỗi này chỉ lộ ra sau khi có bài bị xoá trong DB.
                const totalLessons =
                  chapter.lessons?.length ?? chapter.totalLessons ?? 0;
                const progress = getChapterProgress
                  ? getChapterProgress(chapter.id, totalLessons, lessonIds)
                    ...
                    <div className="chapter-card-info">
                      ...
                      <div className="chapter-card-footer">
                        ...
                        {isCompleted ? (
                          <button
                            type="button"
                            className="btn-chapter-pill done"
                          >
                            <RotateCcw size={13} />
                            <span>Ôn lại</span>
                          </button>
                          ...
                          <button
                            type="button"
                            className="btn-chapter-pill continue"
                          >
                            <span>Học tiếp</span>
                            <ArrowRight size={13} />
                          </button>
