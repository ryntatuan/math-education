...
const CASES = [
  ...
  ["TenFrame", TenFrame, { filled: 7, total: 10, emoji: "🔴", extra: 3, label: "7 + 3 = 10" }],
  ["BaseTenBlocks", BaseTenBlocks, { tens: 3, ones: 4, label: "34" }],
  ["PlaceValueTable", PlaceValueTable, { headers: ["Trăm", "Chục", "Đơn vị"], digits: [3, 4, 5], highlight: 1 }],
  ["Ruler", Ruler, { lengthCm: 10, measure: { from: 0, to: 6 }, unit: "cm" }],
  ["Money", Money, { notes: [20000, 5000, 2000] }],
  ["Table", Table, { headers: ["Đối tượng", "Số lượng"], rows: [["Bóng đá", 12], ["Cầu lông", 8]] }],
  ["PlaneShape", PlaneShape, { kind: "parallelogram", labels: ["8 cm", "5 cm"], formula: "S = 8 × 5 = 40 cm²" }],
  ...
  ["BarModel", BarModel, { rows: [{ label: "Số bé", parts: 2 }, { label: "Số lớn", parts: 6 }], braceLabel: "Tổng: 96" }],
  ["MotionDiagram", MotionDiagram, { mode: "toward", distance: 60, unit: "km", a: { name: "Ô tô", speed: 40 }, b: { name: "Xe máy", speed: 20 } }],
  ["BarChart", BarChart, { title: "Số bạn thích mỗi môn", items: [{ label: "Toán", value: 12 }, { label: "Vẽ", value: 8 }], unit: " bạn" }],
  ["PieChart", PieChart, { title: "Tỉ lệ", items: [{ label: "Toán", percent: 25 }, { label: "Khác", percent: 75 }] }],
];
...
for (const [name, Comp, props] of CASES) {
  for (const [tag, p] of [["mặc định", {}], ["dữ liệu thật", props], ...HOSTILE.map((h, i) => [`độc hại #${i + 1}`, h])]) {
    try {
      ...
      else if (html.length < 60) failures.push(`${name} [${tag}]: quá ngắn (${html.length})`);
      else pass++;
    } catch (e) {
...
try {
  const none = renderToStaticMarkup(VisualBlocks({ content: { text: "chỉ có chữ" } }));
  if (none !== "") failures.push(`VisualBlocks: nội dung không có hình mà vẫn trả về "${none.slice(0, 40)}"`);
  else pass++;
  ...
  if (svgCount < 3) failures.push(`VisualBlocks: chờ 3 hình, chỉ thấy ${svgCount}`);
  else pass++;

  if (demHinh({ numberLine: {}, money: {} }) !== 2) failures.push("demHinh: đếm sai");
  else pass++;
  ...
  for (const bad of [null, undefined, 42, "chuỗi", []]) {
    ...
    if (out !== "") failures.push(`VisualBlocks: content=${JSON.stringify(bad)} mà trả về nội dung`);
    else pass++;
  }
} catch (e) {
